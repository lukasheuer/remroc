...
class ExperimentAsync(Node):
        def __init__(self):
                ...
                with open(Path(self.pkg_remroc).joinpath('params', self.experiment_yaml)) as file:
                ...
                for robot_name in self.robot_names:
                        self.create_subscription(PoseStamped, f'{robot_name}/robot_state', lambda msg, in_robot_name=robot_name: self.state_subscriber_callback(msg, in_robot_name), 10)

                # Subscribers to the odometry/filtered topic to collect some metrics
                for robot_name in self.robot_names:
                        self.create_subscription(Odometry, f'{robot_name}/odometry/filtered', lambda msg, in_robot_name=robot_name: self.odom_subscriber_callback(msg, in_robot_name), 10)

                # Subscribers to the odometry/filtered topic to collect some metrics
                for robot_name in self.robot_names:
                        self.create_subscription(LaserScan, f'{robot_name}/laser_scan', lambda msg, in_robot_name=robot_name: self.laser_subscriber_callback(msg, in_robot_name), 10)
                ...
                with open(Path(self.pkg_remroc).joinpath('params', self.mapf_yaml)) as mapf_file:
                ...
                for x in range(yaml_file['map']['dimensions'][0]):
                        for y in range(yaml_file['map']['dimensions'][1]):
                                ...
                                self.centroid_dict[f'{x}, {-(y - temp_index)}'] = [
                                        map_resolution * (x * grid_cell_size + x_offset + grid_cell_size / 2) + map_origin[0],
                                        map_resolution * (y * grid_cell_size + y_offset + grid_cell_size / 2) + map_origin[1],
                                ]
        ...
        def update_robot_centroids(self):
                ...
                for robot, state in self.robot_states.items():
                        ...
                        # Current Robot position as recieved from the message (meters in map frame)
                        robot_position = np.array([state.pose.position.x, state.pose.position.y])
                        ...
                        # check if robot centroid changed
                        if self.robot_centroids[id] != robot_centroid_index:
        ...
        def send_mapf_request(self):
                ...
                if not self.update_robot_centroids():
                        return None
        ...
        def send_action_goal(self, mapf_response):
                ...
                if valid_mapf_plan:
                        ...
                        for robot_name, action_client in self.action_client_dict.items():
                                ...
                                # this makes it such that the current robot cell is not part of the path. this helps with stop and go behaviour
                                if len(robots_array_dict[int(robot_name.split('_')[1])]) > 1:
                                        start_index = 1
                                else:
                                        start_index = 0
                                for pose in robots_array_dict[int(robot_name.split('_')[1])][start_index:]:
...
def main():
        ...
        while not robots_at_goal:
                ...
                if experiment.get_clock().now() - loop_start_time >= experiment.target_coordinator_loop_time:
                        ...
                        if response is None:
                                continue
                        experiment.get_logger().info('Response recieved.')
                        ...
                        experiment.get_logger().info('Goal send.')
